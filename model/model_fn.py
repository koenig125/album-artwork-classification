...
def build_model(is_training, inputs, params):
    ...
    assert out.get_shape().as_list() == [None, np, np, num_channels * 2]

    out = tf.reshape(out, [-1, np * np * num_channels * 2])
    with tf.variable_scope('fc_1'):
        out = tf.layers.dense(out, num_channels * 2, kernel_regularizer=regularizer)
        if params.use_batch_norm:
            out = tf.layers.batch_normalization(out, momentum=bn_momentum, training=is_training)
        out = tf.nn.relu(out)
